...
async function insertOutbound({ business, contact, body, kind, templateName, buttons, status, providerId, phoneNumberId }) {
  ...
  await db
    .prepare("UPDATE contacts SET last_activity_at = CURRENT_TIMESTAMP WHERE id = ?")
    .run(contact.id);
  return row;
}
...
export async function recordInbound({
    ...
    const parent = await db
      .prepare(
        `SELECT id FROM messages
          WHERE contact_id = ? AND direction = 'out' AND kind = 'interactive_buttons'
  ...
  const row = await db
    ...
    .get(business.id, contact.id, body, kind, providerId, replyToId, phoneNumberId);
  await db
    .prepare("UPDATE contacts SET last_activity_at = CURRENT_TIMESTAMP WHERE id = ?")
    .run(contact.id);
  return row;
